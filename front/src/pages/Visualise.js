...
const Visualise = () => {
  ...
  const handleClick = (feedbackCurrent,category) => {
    ...
    if (currentIndex < recommended.length) {
      ...
      axios.post('http://localhost:8001/submit-feedback', feedback)
      .then(response => {
        console.log('Feedback submitted:', response.data);
        axios.post('http://localhost:8001/get_recommendations', original, {
          headers: {
  ...
  const handleSearch = (event) =>
  {
    ...
    axios.post('http://localhost:8001/get_images', { query:search })
        .then(response => {
          console.log('Feedback submitted:', response.data);
  ...
  >
    <svg className="h-16 w-16 text-red-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      ...
      <line x1="9" y1="9" x2="15" y2="15" />
          ...
          {current ? (
            <img src={`http://127.0.0.1:8001/fitted_images/${current}`} alt="Current" className="w-full h-full object-cover" style={{ borderRadius: '1em' }} />
          ) : (
            <p>Loading...</p>
          )}
        </Grid>
