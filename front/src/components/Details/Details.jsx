  ...
  const {data,setData,selectedImage,setSelectedImage,responseImages,setResponseImages,intial,setInitial,original,setOriginal,recommended,setRecommended,showNext,setShowNext,current,setCurrent,details,setDetails} = useContext(ImageContext)
    const sizes = ['XS','S','M','L','XL']
      ...
      const handleSubmit = async (item) => {
        ...
        try {
          const userImageCheck = await axios.get('http://localhost:8001/check_user_image');
          if (!userImageCheck.data.has_user_image) {
            Swal.fire({
              icon: 'warning',
              title: 'Upload Required',
              text: 'Please upload your photo first to try on clothes!',
          ...
          Swal.fire({
            icon: 'error',
            title: 'Error',
            text: 'Unable to check user image status. Please upload your photo first.',
          });
          return;
        }
        
        axios.post('http://localhost:8001/get_recommendations', item, {
          headers: {
